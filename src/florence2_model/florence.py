...
FLORENCE_OPEN_VOCABULARY_TASK = '<OPEN_VOCABULARY_DETECTION>'
...
def load_florence_model(
      ...
      with patch("transformers.dynamic_module_utils.get_imports", fixed_get_imports):
            model = AutoModelForCausalLM.from_pretrained(checkpoint, torch_dtype=torch.bfloat16, trust_remote_code=True).to(device).eval()
...
def run_florence_inference(
      ...
      parsed_output = processor.post_process_generation(generated_text, task=task, image_size = image.size)
